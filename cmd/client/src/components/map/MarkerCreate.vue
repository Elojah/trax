...
// Convert bigint coordinates to human-readable format
const formatCoordinates = computed(() => {
	if (!props.coordinates) return '';
	const lat = Number(props.coordinates.lat); // Assuming coordinates are in micro-degrees
	const lng = Number(props.coordinates.lng);
		...
		<div class="flex flex-col h-full">
			...
			<div class="px-6 py-4 border-b border-surface flex items-center justify-between flex-shrink-0">
				<div class="flex items-center gap-4">
					<div class="flex items-center gap-2">
						...
						<img :src="catpaw" alt="Cat Paw" class="h-8 w-8" />
					</div>
			...
			<div class="flex-1 overflow-y-auto">
				<div class="flex flex-col">
					...
					<div class="flex flex-col">
						<div class="px-6 py-3 bg-surface-50 dark:bg-surface-800">
							<span class="text-surface-900 dark:text-surface-0 font-medium">Location</span>
						</div>
						<div class="p-6 flex flex-col gap-4">
							<!-- Coordinates display -->
							<div v-if="coordinates" class="flex gap-2 text-sm text-surface-600 dark:text-surface-400">
								<span class="font-medium">Coordinates:</span>
								<span>{{ formatCoordinates }}</span>
							</div>

							<!-- Address with loading state -->
							<div class="flex flex-col gap-2">
								<label class="text-surface-900 dark:text-surface-0 text-sm font-medium">Address</label>
								<div v-if="geocodingLoading"
									class="flex items-center gap-2 p-3 border border-surface-200 dark:border-surface-700 rounded-lg bg-surface-50 dark:bg-surface-800">
									<ProgressSpinner style="width: 16px; height: 16px" stroke-width="4" />
									<span class="text-surface-600 dark:text-surface-400 text-sm">Looking up
										address...</span>
								</div>
								<div v-else-if="geocodingError" class="flex flex-col gap-2">
									<Message severity="warn" :closable="false">
										Failed to load address: {{ geocodingError }}
									</Message>
									<InputText :value="formatCoordinates" placeholder="Enter address manually"
										readonly />
								</div>
								<InputText v-else :value="address" placeholder="Address will appear here..." readonly />
							</div>
						</div>
					</div>
